...
% WIKI the default page size for all standard document classes is US letter.
...
% XXX taken straight from the tempale \LaTeX file for NeurIPS 2020
...
% XXX a rather cumbersome hack to make [citation-needed] a citeable bibobject with
%  quite noticeable colour
\bibliographystyle{abbrvnat}
\begingroup
% repurposed from this SE reply
%   https://tex.stackexchange.com/questions/464284/add-bbl-file-entries-to-latex-document
\makeatletter
...
\bibitem[{\color{magenta}{\textbf{citation needed}}}()citation-needed]{citation_needed}
\attn{[citation-needed]}
\newblock
    ``A bold colourful bibliography item, which indicates to the contributors that
    a statement or a claim requires verification or is missing a supporting citation.''
\end{thebibliography}
...
% XXX use this bibliography when the text is no logern a draft
% \bibliographystyle{abbrvnat}
% \bibliography{main}

\appendix
\section{More details of the ablation study} % (fold)
...
\begin{table}[!t]
% XXX Note that publication-quality tables \emph{do not contain vertical rules.} We
...
\subsection{World models with inverse dynamics and autoencoders} % (fold)
\label{sec:wm_idm_ae}
...
%   \includegraphics[draft]{non_existent_figure.pdf}

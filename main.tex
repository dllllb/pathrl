...
The goal-setter $G$ is trained with four rewards: the extrinsic reward, i.e. true reward of the environment; the intrinsic curiosity reward for active exploration; non-reachable goal penalty, received if the worker is not achieved the generated goal in k steps; and short goals penalty, for goals, achieved in less than c steps.
...
\subsubsection{Additional training for the navigator}

The navigator can be additionally trained during the second phase. During the navigator training, the navigator weights are unfrozen. The navigator is trained on the intrinsic rewards of reaching the generated goals. The navigator does not receive reward for too complex goals, when goal is not achieved in k steps. The goal-setter receives only exploration reward during the worker training.

If additional training for the navigator is used, than it can possibly replace the first phase of the algorithm. The fist phase acts as a navigator pre-training step in this setup.

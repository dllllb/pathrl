...
Another possible approach to extract state entities is to train the global state encoder to produce a fixed set of the discrete vectors or global vectors. The global vectors will be used for the restoration of the entire sequience of states along some trajectory of the state-action pairs. The global vectors are produced by encoding the initial state of the trajectory. The state restoration model takes the fixed set of vectors from encoder, the \it{tiny} embedding of the current state and the current action as the input. The state difference loss is used to train current state encoder, the global state encoder and the decoder, and the state restoration model. Since, the current state encoder output is tiny, the system is encoraged to store information, that is shared between the trajectory states in the global vectors. The VQ-VAE like approach can be used to make global vectors discrete.
